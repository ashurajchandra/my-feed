//Import NPM packages   
import React from 'react';
import { Link } from 'react-router-dom';

//Import components
import Header from '../../components/Header/Header';

//Import constants
import { LABEL, PLACEHOLDER } from './constants';

//Import images
import IMAGES  from '../../assets/images';

const SignUp: React.FC = () => {
  return (
    <div>
    <Header image={IMAGES.DEFAULT_AVATAR} username="foo-rum" logout={() => {}} />
    {/* SignIn Component - Central Card */}
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-80px)] px-4">
      <div className="w-[498px] h-[653px] pt-[11px] pb-[55px] pl-[11px] pr-[11px] bg-[#EBEBEB] rounded-[30px]">
        <div className=" w-full h-full bg-[#FFFFFF] rounded-[21px] shadow-[0px_4px_15px_0px_#00000008] pt-[30px] pb-[57px] pl-[50px] pr-[50px] ">

        <div className='flex flex-col items-center justify-center mb-[65px]'> 
            {/* Main Heading */}
            <img src={IMAGES.LOGIN_ICON.src} alt={IMAGES.LOGIN_ICON.alt} />
          <h1 className="text-[20px] font-bold text-[#000000] font-inter text-center mb-[5px]">
            {LABEL.CREATE_ACCOUNT_TO_CONTINUE}
          </h1>
          
          {/* Subtitle */}
          <p className="text-[14px] font-normal text-[#0000007A] text-center">
            {LABEL.CREATE_ACCOUNT_TO_ACCESS_FEATURES}
          </p>
        </div>

          {/* Form */}
          <form>
            {/* Email/Username Field */}
            <div className='mb-[15px]'>
              <label htmlFor="email" className="block text-[14px] font-semibold text-[#000000] mb-[6px]">
                {LABEL.EMAIL_OR_USERNAME}
              </label>
              <input
                type="text"
                id="email"
                name="email"
                className="w-full h-[46px] px-[12px] py-[13px]  rounded-[11px] placeholder-[#0000007A] focus:outline-none bg-[#F4F4F4]"
                placeholder={PLACEHOLDER.EMAIL_OR_USERNAME}
              />
            </div>

            {/* Password Field */}
            <div className='mb-[15px]'>
              <label htmlFor="password" className="block text-[14px] font-semibold text-[#000000] mb-[10px]">
                {LABEL.PASSWORD}
              </label>
              <input
                type="password"
                id="password"
                name="password"
                className="w-full h-[46px] px-[12px] py-[13px]  rounded-[11px] placeholder-[#0000007A] focus:outline-none bg-[#F4F4F4]"
                placeholder={PLACEHOLDER.PASSWORD}
              />
            </div>
            <div className='mb-[22px]'>
              <label htmlFor="password" className="block text-[14px] font-semibold text-[#000000] mb-[10px]">
                {LABEL.REPEAT_PASSWORD}
              </label>
              <input
                type="password"
                id="password"
                name="password"
                className="w-full h-[46px] px-[12px] py-[13px]  rounded-[11px] placeholder-[#0000007A] focus:outline-none bg-[#F4F4F4]"
                placeholder={PLACEHOLDER.ENTER_PASSWORD_AGAIN}
              />
            </div>

            {/* Sign In Button */}
            <button
              type="submit"
              className="w-full h-[50px] rounded-[11px] bg-[#5057EA] text-[#FFFFFF] text-[14px] font-semibold font-inter  focus:outline-none"
            >
              {LABEL.SIGN_UP}
            </button>
          </form>

      
        </div>
          {/* Sign Up Link */}
          <div className="mt-[15px] mb-[19px] text-center">
            <span className="text-[#00000099] text-[14px] font-medium font-inter">{LABEL.ALREADY_HAVE_AN_ACCOUNT} </span>
            <Link to="/signin" className="text-[#5057EA] text-[14px] font-semibold font-inter">
              {LABEL.SIGN_IN}
            </Link>
          </div>
      </div>
        
    </div>
  </div>
  );
};

export default SignUp;
