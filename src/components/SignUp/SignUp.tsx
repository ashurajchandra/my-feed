...
const SignUp: React.FC<{ updatePopUpContent?: () => void }> = ({ updatePopUpContent }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const username = isEmail ? '' : formData.emailOrUsername;
    
    try {
      await signup(email, username, formData.password);
      navigate('/');
    } catch (err) {
  ...
  return (
    ...
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-80px)] px-4">
      <div className="w-[498px] h-[653px] pt-[11px] pb-[55px] pl-[11px] pr-[11px] bg-[#EBEBEB] rounded-[30px]">
        ...
        <div className='flex flex-col items-center justify-center mb-[65px]'> 
          ...
          <h1 className="text-[20px] font-bold text-[#000000] font-inter text-center mb-[5px]">
            {LABEL.CREATE_ACCOUNT_TO_CONTINUE}
          </h1>
          
          {/* Subtitle */}
          <p className="text-[14px] font-normal text-[#0000007A] text-center">
            {LABEL.CREATE_ACCOUNT_TO_ACCESS_FEATURES}
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className='mb-[22px]'>
              <label htmlFor="confirmPassword" className="block text-[14px] font-semibold text-[#000000] mb-[10px]">
              ...
              <input
                ...
                onChange={handleInputChange}
                className="w-full h-[46px] px-[12px] py-[13px]  rounded-[11px] placeholder-[#0000007A] focus:outline-none bg-[#F4F4F4]"
                placeholder={PLACEHOLDER.ENTER_PASSWORD_AGAIN}
                minLength={6}
                required
              />
            </div>
          ...
          <div className="mt-[15px] mb-[19px] text-center">
            <span className="text-[#00000099] text-[14px] font-medium font-inter">{LABEL.ALREADY_HAVE_AN_ACCOUNT} </span>
            {updatePopUpContent ? 
              <button className="text-[#5057EA] text-[14px] font-semibold font-inter outline-none bg-transparent" onClick={updatePopUpContent}>
                {LABEL.SIGN_IN}
              </button>
              :
                <Link to="/signin" className="text-[#5057EA] text-[14px] font-semibold font-inter">
                {LABEL.SIGN_IN}
              </Link>
            }
          </div>
