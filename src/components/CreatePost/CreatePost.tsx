...
interface PostData {
  content: string;
  format:
    | typeof FORMATTING_OPTIONS.PARAGRAPH
    | typeof FORMATTING_OPTIONS.HEADING_1
    | typeof FORMATTING_OPTIONS.HEADING_2
    | typeof FORMATTING_OPTIONS.HEADING_3;
  isBold: boolean;
  isItalic: boolean;
  isUnderline: boolean;
  listType:
    | typeof FORMATTING_OPTIONS.NONE
    | typeof FORMATTING_OPTIONS.UNORDERED_LIST
    | typeof FORMATTING_OPTIONS.ORDERED_LIST;
}
...
const CreatePost: React.FC<CreatePostProps> = ({
  ...
  const toggleFormat = (
    format: keyof Pick<PostData, "isBold" | "isItalic" | "isUnderline">
  ) => {
    setPostData((prev) => {
  ...
  const handleButtonClick = (button: any) => {
    switch (button.onClick) {
      case "toggleFormat":
        toggleFormat(
          button.formatKey as keyof Pick<
            PostData,
  ...
  const getTextareaStyle = () => {
    let style =
      "resize-none w-full min-h-[101px] h-[101px] text-gray-800 placeholder-gray-400 focus:outline-none";
